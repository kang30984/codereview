...
async function runReview() {
    try {
        ...
        let targetSrcFiles = allFiles.filter(file => {
            const inMain = path.relative(TARGET_DIR, file).split(path.sep).includes('main');
            const ext = path.extname(file).toLowerCase();
            return inMain && validExtensions.includes(ext);
        });
        ...
        const statsCount = { safe: processedFiles.size, vulnerable: existingVulnerabilities.length, error: 0, skipped: 0 };
        ...
        const updateDisplay = () => {
            ...
            process.stdout.write(`\x1B[1;32m[통계]\x1B[0m 안전: ${statsCount.safe} | \x1B[1;31m취약: ${statsCount.vulnerable}\x1B[0m | 에러/환각: ${statsCount.error} | 스킵: ${statsCount.skipped}\n\n`);
...
명심하십시오: 취약점이 없으면 오직 {"vulnerable": false}만 출력해야 합니다. 코드 요약, 메소드 설명, 예제 데이터 생성 등은 절대 금지됩니다.`;
        ...
        logger.step(`모든 작업 완료! 리포트: ${REPORT_FILE}`);
